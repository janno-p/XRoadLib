...
internal sealed class XRoadMessageReader : IDisposable
{
    ...
    private static byte[] DecodeFromBase64(byte[] buffer, Encoding encoding)
    {
        ...
        return buffer.Length > 0 ? Convert.FromBase64CharArray(encoding.GetChars(buffer), 0, encoding.GetCharCount(buffer)) : Array.Empty<byte>();
    }
