...
public class ServiceMap : IServiceMap
{
    ...
    private async Task<object> DeserializeValueAsync(XmlReader reader, ITypeMap typeMap, IXmlTemplateNode templateNode, ParticleDefinition particleDefinition, XRoadMessage message)
    {
        ...
        XName typeAttribute;
        if (typeMap.Definition.IsAnonymous && typeMap is not IArrayTypeMap && (typeAttribute = reader.GetTypeAttributeValue()) != null)
    ...
    /// </summary>
    public async Task SerializeResponseAsync(XmlWriter writer, object value, XRoadMessage message, XmlReader requestReader, ICustomSerialization customSerialization = null)
    {
