...
public sealed class ServiceDescriptionBuilder
{
    ...
    private void CollectTypes()
    {
        ...
        foreach (var typeDefinition in typeDefinitions)
        {
            ...
            if (!_derivedTypes.TryGetValue(baseType, out var typeList))
            {
                ...
                _derivedTypes.Add(baseType, typeList);
            }
    ...
    private IEnumerable<XmlSchema> BuildSchemas(string targetNamespace, MessageCollection messages)
    {
        ...
        while (newTypesResolved)
        {
            ...
            foreach (var key in schemaReferences.SelectMany(x => x.Value.Types).Where(x => x.Value == null).Select(kvp => kvp.Key).ToList())
            {
                var schemaTypeName = XName.Get(key.Name, key.Namespace);
    ...
    private Tuple<XmlSchemaElement, XmlSchemaElement> CreateXmlSchemaElement(XName name, SchemaReferences schemaReferences)
    {
        ...
        var elementForm = _schemaDefinitionProvider.IsQualifiedElementDefault(ns)
            ? GetUnqualifiedElementForm(elementNamespace)
            : GetQualifiedElementForm(elementNamespace, ns);
    ...
    private static XmlSchemaForm? GetUnqualifiedElementForm(string elementNamespace) =>
        elementNamespace == "" ? XmlSchemaForm.Unqualified : null;

    private static XmlSchemaForm? GetQualifiedElementForm(string elementNamespace, string ns) =>
        elementNamespace == ns ? XmlSchemaForm.Qualified : null;
    ...
    private Tuple<XmlSchemaElement, XmlSchemaElement> SetXmlSchemaElementName(Tuple<XmlSchemaElement, XmlSchemaElement> elements, XName name, SchemaReferences schemaReferences)
    {
        ...
        var elementForm = _schemaDefinitionProvider.IsQualifiedElementDefault(ns)
            ? GetUnqualifiedElementForm(name.NamespaceName)
            : GetQualifiedElementForm(name.NamespaceName, ns);
    ...
    private sealed class SchemaReferences
    {
